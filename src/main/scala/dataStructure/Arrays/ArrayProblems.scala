...
object ArrayProblems extends App{
  ...
  def removeDuplicates(arr: Array[Int]): Array[Int] =
    arr.foldLeft(List[Int]())((acc, x) => if(!acc.contains(x)) x :: acc else acc).toArray
  ...
  private def printDuplicate(arr: Array[Int]): Unit ={
    val duplicateElements = arr.groupBy(identity).filter(_._2.length > 1).keys
    println("Duplicate elements in the array: " + duplicateElements)
  }
  ...
  def rotateRight(nums: Array[Int], k: Int): Array[Int] = {
    val kMod = k % nums.length
    val split = nums.splitAt(nums.length - kMod)
    split._2 ++ split._1
  }
  ...
  def union(arr1: Array[Int], arr2: Array[Int]): Unit = {
    val union = (arr1 ++ arr2).foldLeft(List[Int]()) {
           case (acc, x) if !acc.contains(x) => x :: acc
           case (acc, _) => acc }.reverse.toArray
  }
  ...
  def findFirstRepeat(nums: Array[Int]): Int = {
    for (i <- nums.indices) {
